<?php
			...
			$lcMainKeyword = '';
			if(isset($this->options['unicode_support']))
			{
				$lcMainKeyword = (isset($this->postMeta['onpageseo_global_settings']) && isset($this->postMeta['onpageseo_global_settings']['MainKeyword'])) ? mb_strtolower($this->postMeta['onpageseo_global_settings']['MainKeyword'], 'UTF-8') : '';
			}
			else
			{
				$lcMainKeyword = (isset($this->postMeta['onpageseo_global_settings']) && isset($this->postMeta['onpageseo_global_settings']['MainKeyword'])) ? strtolower($this->postMeta['onpageseo_global_settings']['MainKeyword']) : '';
			}
	...
	jQuery('#reload-lsi-keywords').click(function(){
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if(response)
			{
				...
				var data = {
					action: 'onpageseo_bing_keywords',
				...
				jQuery.post(ajaxurl, data, function(response) {
					...
					if(response)
					{
						...
						if(ajaxResponse['error'] !== undefined)
						{
							...
							{
								jQuery('.onpageseoerror').html('<span>Error:</span> Unable to connect to service. Please, check your API key in the settings.');

							}
							else
							{
								jQuery('.onpageseoerror').html('<span>Error:</span> No relevant keywords found.');
							}
						...
						{
							lsiKeywords = ajaxResponse['keywords'];
							var i = 0;
							lsiKeywordDisplay += '<ul id="sortable">';
							for (var i = 0; i < lsiKeywords.length; i++)
							{
								var highlight = ''

								if(wpEditorText.toLowerCase().indexOf(jQuery.trim(lsiKeywords[i])) >= 0)
								{
									highlight = ' lsihighlight';
								}

								lsiKeywordDisplay += '<li class="draggables'+highlight+'" id="lsikeyword'+i+'">' + lsiKeywords[i] + '</li>';
								if(i >= (<?php echo $this->options['lsi_keyword_maximum_results'];?> - 1)) break;
							}
							lsiKeywordDisplay += lsiSecondaryKeywordDisplay + '</ul>';
