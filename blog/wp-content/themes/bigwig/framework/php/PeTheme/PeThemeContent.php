...
class PeThemeContent {
	...
	public function filter($tax,$sep = "",$aclass="label",$template = '%s') {
		...
		if (is_array($terms) && count($terms) > 0) {
			$buffer = apply_filters("pe_theme_filter_item",sprintf('<a class="%s active" data-category="" href="#">%s</a>%s',$aclass,__pe("All"),"$sep\n"),$aclass,"",__pe("All"));
	...
	public function the_content_more_link_filter($link) {
		$link = sprintf('&nbsp;<a href="%s#more-%s" class="read-more">%s</a>',get_permalink(),$GLOBALS["post"]->ID,__pe("more"));
		return $link;
	}
